...
top        = max(Node(2,:)); %ymax
C_char = R_char*G_char*L_char/M_char;
surface_load = rho_ice*grav*h_ice*rhs_const;
dnm = 1/(2*mju+lan);
dnms= 1/(mju*(2*mju/lan+1));
alfa = rho_earth*grav*dnm;
beta=rho_ice*grav*h_ice*dnms;
if vec_coeff(2)==0,  % no prestress
    gradV=-surface_load*dnms;   % scaled pressure no prestress
    [max(UVPp) gradV max(UVPp)-gradV]
    UVPv_exact = surface_load*dnm*(bottom-Node(2,:))';
    UVPp_exact = gradV*ones(nnodeP,1);
else % ------------------- with prestress
    UVPv_exact = -rho_ice*h_ice/rho_earth*L_char*(exp(alfa*(Node(2,:)-top)*C_char) - ...
                                                                                          exp(alfa*(bottom   -top)*C_char))';
    UVPp_exact =  -beta*C_char*exp(alfa*(Node(2,1:nnodeP)-top)*C_char)';
end
figure(5),clf,plot3(Node(1,:),Node(2,:),UVPv_exact-U_char*UVPv*L_char,'v');
...
while (time_cur<=Tmax) || (norm_diff>1e-5)
    ...
    %% ---  Checks for the solution in the case of uniform load
     ...
     exb = exp(-Maxwell_time_inv*time_cur);
    if vec_coeff(2)==0,  % no prestress
...
%        UVPv_exact = rho_ice*h_ice*grav*dnm*C_char*alfas/(alfas+1)*(bottom-Node(2,:)')*L_char*(1+1/alfas*exp(-(1+alfas)*time_cur)); 
    ...
    UVPv_exact = -rho_ice*h_ice/rho_earth*L_char*exp(2*Maxwell_time_inv*time_cur)*...
                              ( exp(alfat*(Node(2,:)-top)*C_char) - exp(alfat*(bottom -top)*C_char))';
    UVPp_exact =  -betat*C_char*exp(alfat*(Node(2,1:nnodeP)-top)*C_char)';
    end
