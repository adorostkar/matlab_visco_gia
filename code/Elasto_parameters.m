...
function [L,H,l_ice,h_ice,rho_ice,rho_earth,...
    ...
    if domains==2, E_domains(2,1)=E_domains(2,1)*Emagn; end
    ...
    rho_ice0   = 1;   % kg/m^3
    rho_earth0 = 1;   % kg/m^3
    nju(1)     = 0.4;   % dimensionless
    nju(2)     = 0.4;   % dimensionless (to be varied)
    eta0       = 1.0e+21;  % Viscosity Pa s
    grav0      = 1;     % m/s^2
    ...
    L_char  = L0;      %L_char=max(abs(L),abs(H));
    ...
    mju                  = mju0/M_char;
    Maxwell_time_inv     = 0.01; % mju0(1:domains)'/eta0*T_char;
